...
public class FileSplit_aes {
    public static void main(String[] args) throws IOException {
        new FileSplit_aes().splitFile(new File(args[0]));
    }
    ...
    private void writeBlock(int blockNumber, byte[] buffer, int length) throws IOException {
        FileOutputStream fos = new FileOutputStream("split_aes/output" + blockNumber + ".txt");
        try {
            fos.write(buffer, 0, length);
            System.out.println("\n writing in file " +blockNumber);
        }
        finally {
            fos.close();
        }
